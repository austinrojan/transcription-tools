...
from transcription_tools.install_paths import FFMPEG_DIR


class TestInstallDirFfmpegCandidates:
    def test_install_dir_ffmpeg_is_first_candidate(self):
        assert FFMPEG_CANDIDATES[0] == str(FFMPEG_DIR / "ffmpeg")

    def test_install_dir_ffprobe_is_first_candidate(self):
        assert FFPROBE_CANDIDATES[0] == str(FFMPEG_DIR / "ffprobe")
...
class TestFindFfprobe:
    ...
    def test_raises_when_not_found(self):
        with (
            patch("shutil.which", return_value=None),
            patch.object(Path, "is_file", return_value=False),
            pytest.raises(FileNotFoundError, match="ffprobe not found"),
        ):
            find_ffprobe()
...
class TestProbeAudioStreams:
    ...
    def test_returns_audio_streams(self, mock_subproc, mock_copy, _mock_ffprobe):
        ...
        mock_result.stdout = json.dumps({"streams": [{"codec_type": "audio", "codec_name": "aac"}]})
...
class TestValidateHasAudio:
    @patch("transcription_tools.audio.probe_audio_streams", return_value=[{"codec_type": "audio"}])
...
class TestConvertToWav:
    ...
    def test_raises_runtime_error_on_ffmpeg_failure(
        ...
        mock_subproc.run.side_effect = subprocess.CalledProcessError(
            1,
            "ffmpeg",
            stderr=b"conversion error details",
        )
    ...
    def test_output_temp_cleaned_up_on_failure(
        ...
        with patch("transcription_tools.audio.Path") as MockPath, pytest.raises(RuntimeError):
            convert_to_wav("/input/audio.mp3")
